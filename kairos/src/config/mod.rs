...
pub fn deserialize_config(path: &Path, warn_pruned: bool) -> Result<Value> {
    ...
    if (extension == "yaml" || extension == "yml") && !contents.trim().is_empty() {
        warn!(
            "YAML config {path:?} is deprecated, please migrate to TOML using `kairos migrate`"
        );
...
pub fn installed_config(suffix: &str) -> Option<PathBuf> {
    let file_name = format!("kairos.{suffix}");

    // Try using XDG location by default.
    xdg::BaseDirectories::with_prefix("kairos")
        .find_config_file(&file_name)
        .or_else(|| xdg::BaseDirectories::new().find_config_file(&file_name))
        .or_else(|| {
            if let Ok(home) = env::var("HOME") {
                // Fallback path: $HOME/.config/kairos/kairos.toml.
                let fallback = PathBuf::from(&home).join(".config/kairos").join(&file_name);
                if fallback.exists() {
                    return Some(fallback);
                }
                // Fallback path: $HOME/.kairos.toml.
            ...
            let fallback = PathBuf::from("/etc/kairos").join(&file_name);
...
pub fn installed_config(suffix: &str) -> Option<PathBuf> {
    let file_name = format!("kairos.{suffix}");
    dirs::config_dir().map(|path| path.join("kairos").join(file_name)).filter(|new| new.exists())
}
