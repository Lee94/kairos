...
use kairos_terminal::index::Point;
...
#[cfg(all(feature = "x11", not(any(target_os = "macos", windows))))]
const WINDOW_ICON: &[u8] = include_bytes!("../../extra/logo/compat/kairos-term.png");

/// This should match the definition of IDI_ICON from `kairos.rc`.
#[cfg(windows)]
...
///
/// Wraps the underlying windowing library to provide a stable API in Kairos.
pub struct Window {
...
impl Window {
    ...
    pub fn update_ime_position(&self, point: Point<usize>, size: &SizeInfo) {
        ...
        // Anchor relative to the grid's origin (chrome offsets and, for split panes, the pane's
        // position included), not the bare window padding.
        let nspot_x = f64::from(size.grid_left() + point.column.0 as f32 * size.cell_width());
        let nspot_y =
            f64::from(size.grid_top() + (point.line + offset) as f32 * size.cell_height());
    ...
    /// Anchor the IME candidate window to a window-pixel rectangle (used by the chrome's command
    /// palette, which lives outside the terminal grid).
    pub fn set_ime_cursor_px(&self, x: f32, y: f32, width: f32, height: f32) {
        self.window.set_ime_cursor_area(
