...
pub struct SizeInfo<T = f32> {
    ...
    /// Extra pixels reserved at the right edge of the window beyond this grid's area. Non-zero
    /// only for split panes that don't touch the window's right edge.
    right_extra: T,

    /// Extra pixels reserved at the bottom edge of the window beyond this grid's area. Non-zero
    /// only for split panes that don't touch the window's bottom edge.
    bottom_extra: T,
