...
const KAIROS_SOCKET_ENV: &str = "KAIROS_SOCKET";
...
impl IpcListener {
    pub fn new(
        ...
        // Register socket path as environment variable for `kairos msg`.
        unsafe { env::set_var(KAIROS_SOCKET_ENV, path.as_os_str()) };
        if options.daemon {
            println!("KAIROS_SOCKET={}; export KAIROS_SOCKET", path.display());
        }
...
/// Send a message to the active Kairos socket.
...
fn find_socket(socket_path: Option<PathBuf>) -> IoResult<UnixStream> {
    ...
    if let Ok(path) = env::var(KAIROS_SOCKET_ENV) {
    ...
    for entry in fs::read_dir(socket_dir())?.filter_map(|entry| entry.ok()) {
        ...
        // Skip files that aren't Kairos sockets.
