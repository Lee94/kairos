...
fn update_projection(u_projection: GLint, size: &SizeInfo) {
    ...
    // Pixels reserved around the grid: chrome tab bar (top), project sidebar (left) and — for
    // split panes — the space taken by other panes (right/bottom). The grid's drawable area
    // shrinks by them; the projection must match `Renderer::set_viewport`.
