...
impl WindowContext {
    ...
    pub fn close_tab(&mut self, proxy: EventLoopProxy<Event>, terminal_id: u64) -> bool {
    ...
    /// turns into a [`Self::close_tab`] (which respawns a fresh shell when it was the project's last
    /// tab). Targeting by id (rather than index) keeps this correct even if tabs shift between click
    /// and handling.
    ...
    fn tab_bar_info(&self) -> TabBarInfo {
        let project = self.active_project();
        // Prefer the active tab's live working directory (where resolvable), falling back to the
        // project root, so the status bar tracks where the user actually is.
        let cwd =
            self.tab_working_directory(self.active_tab()).or_else(|| project.root.clone());
        TabBarInfo {
    ...
    pub fn handle_event(
        ...
        for queued in events {
            ...
            // Events from an already-closed tab have no valid target; drop them.
            let Some(target) = self.event_target_tab(&queued) else { continue };
            self.process_event(
                target,
                #[cfg(target_os = "macos")]
                event_loop,
    ...
    /// active tab.
    ///
    /// Returns `None` when the originating tab no longer exists: a closed tab's PTY can still
    /// have events (e.g. its final `Title`) queued, and falling back to the active tab would
    /// apply them to an unrelated terminal — renaming someone else's tab. Such events are
    /// dropped instead.
    fn event_target_tab(&self, event: &WinitEvent<Event>) -> Option<(usize, usize)> {
        ...
        Some((self.active_project, self.active_project().active_tab))
    }
