...
use kairos_terminal::event::{Event as TerminalEvent, EventListener, Notify};
use kairos_terminal::event_loop::Notifier;
use kairos_terminal::grid::{BidirectionalIterator, Dimensions, Scroll};
use kairos_terminal::index::{Boundary, Column, Direction, Line, Point, Side};
use kairos_terminal::selection::{Selection, SelectionType};
use kairos_terminal::term::cell::Flags;
use kairos_terminal::term::search::{Match, RegexSearch};
use kairos_terminal::term::{self, ClipboardType, Term, TermMode};
...
impl<'a, N: Notify + 'a, T: EventListener> input::ActionContext<T> for ActionContext<'a, N, T> {
    ...
    fn spawn_new_instance(&mut self) {
        ...
        let kairos = env_args.next().unwrap();

        let mut args: Vec<String> = Vec::new();

        // Reuse the arguments passed to Kairos for the new instance.
