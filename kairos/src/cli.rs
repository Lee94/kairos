...
use kairos_config::SerdeReplace;
use clap::{ArgAction, Args, Parser, Subcommand, ValueHint};
use log::{LevelFilter, error};
use serde::{Deserialize, Serialize};
use toml::Value;

use kairos_terminal::tty::Options as PtyOptions;
...
/// CLI options for the main Kairos executable.
...
pub struct Options {
    ...
    /// X11 window ID to embed Kairos within (decimal or hexadecimal with "0x" prefix).
    #[clap(long)]
    pub embed: Option<String>,

    /// Specify alternative configuration file [default:
    /// $XDG_CONFIG_HOME/kairos/kairos.toml].
    #[cfg(not(any(target_os = "macos", windows)))]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,

    /// Specify alternative configuration file [default: %APPDATA%\kairos\kairos.toml].
    #[cfg(windows)]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,

    /// Specify alternative configuration file [default: $HOME/.config/kairos/kairos.toml].
    #[cfg(target_os = "macos")]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,
...
pub struct WindowIdentity {
    /// Defines the window title [default: Kairos].
    #[clap(short = 'T', short_alias('t'), long)]
    pub title: Option<String>,

    /// Defines window class/app_id on X11/Wayland [default: Kairos].
...
/// Send a message to the Kairos socket.
#[cfg(unix)]
#[derive(Args, Debug)]
...
pub enum SocketMessage {
    /// Create a new window in the same Kairos process.
    CreateWindow(WindowOptions),

    /// Update the Kairos configuration.
    Config(IpcConfig),

    /// Read runtime Kairos configuration.
...
pub struct IpcConfig {
    ...
    #[clap(short, long, allow_hyphen_values = true, env = "KAIROS_WINDOW_ID")]
...
mod tests {
    ...
    fn completions() {
        ...
        for (shell, file) in &[
            (Shell::Bash, "kairos.bash"),
            (Shell::Fish, "kairos.fish"),
            (Shell::Zsh, "_kairos"),
        ] {
            let mut generated = Vec::new();
            clap_complete::generate(*shell, &mut clap, "kairos", &mut generated);
        ...
        //
        // let mut file = File::create("../extra/completions/kairos.bash").unwrap();
        // clap_complete::generate(Shell::Bash, &mut clap, "kairos", &mut file);
        // let mut file = File::create("../extra/completions/kairos.fish").unwrap();
        // clap_complete::generate(Shell::Fish, &mut clap, "kairos", &mut file);
        // let mut file = File::create("../extra/completions/_kairos").unwrap();
        // clap_complete::generate(Shell::Zsh, &mut clap, "kairos", &mut file);
    }
}
